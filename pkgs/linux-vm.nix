{ lib, runCommand, writeScript, writeShellApplication, cpio, linuxPackages
, pkgsStatic, zstd, kernel ? linuxPackages.kernel.override {
...
let
  ...
  setupScript = let
    ...
    inherit (lib.strings)
      concatMapStringsSep concatStringsSep escapeShellArg optionalString
      removePrefix;

    escapeRelativePath = path:
      escapeShellArg ("./" + (lib.strings.removePrefix "/" path));

    # an operation must __either__ be a copy __or__ a symlink creation, but not both
  in assert !any (x: hasAttr "copy" x && hasAttr "target" x)
    (attrValues rootfsFiles);
    ...
    text = concatStringsSep "\n" (
      ...
      (mapAttrsToList (path: value:
        optionalString (value ? target) ''
          mkdir --parent -- "$(dirname -- ${escapeRelativePath path})"
          echo "Linking ${escapeRelativePath value.target} to ${
            escapeRelativePath path
          }"
  ...
  initrd = runCommand "initrd.cpio.zst" {
